...
define('CCHL_URL', plugin_dir_url(__FILE__));
...
function category_autocheck_options_page() {
    if (!current_user_can('manage_options')) {
        wp_die(__('Unauthorized access.', 'cchl'));
    }
    ?>
    <div class="wrap">
        <h1><?php _e('Parent Category AutoCheck Settings', 'cchl'); ?></h1>
        <form method="post" action="options.php">
            <?php
            settings_fields('cchl_settings_group');
...
add_action('admin_init', function() {
    register_setting('cchl_settings_group', 'cchl_settings');
    add_settings_section('cchl_main_section', __('Settings', 'cchl'), null, 'category_autocheck');
    add_settings_field('enable_auto_check', __('Enable Auto Check', 'cchl'), function() {
        $options = get_option('cchl_settings', ['enable_auto_check' => 1]);
        echo '<input type="checkbox" name="cchl_settings[enable_auto_check]" value="1" ' . checked(1, $options['enable_auto_check'], false) . ' />';
    }, 'category_autocheck', 'cchl_main_section');
});
=======
<?php
/*
...
>>>>>>> 0e2f4f2de9208334f17c1675e930ea296fa9abbc
